...
This document records the physically used recovery route. It is evidence of a
single approved restoration after a failed first native-Linux attempt; it is
not standing authorization for another write.
...
| S1Boot accepts `fastboot flash boot` on this handset | `VERIFIED_DEVICE` | The failed first experiment and its rollback both reported `Flash of partition 'boot' requested`, `S1 partID 0x00000003`, and blocks `0x00002000-0x0000bfff`, then `Flash operation complete`. Sanitized records are under `research/device/current/boot/`. |
...
`FASTBOOT_P3_RESTORE`, `PHYSICAL_FASTBOOT_P3_RESTORE`, and
`FAILED_MAINLINE_BOOT_ROLLBACK` are **`VERIFIED_DEVICE`**. After hardware
S1Boot entry remained available following the unusable experimental p3, the
exact original p3 ELF was accepted through the logical `boot` target and the
ScrubbModRom Android baseline booted normally. This directly establishes a
working p3 rollback route for this handset and exact restore artifact.
...
was not parsed or exposed. The historical first attempt and its completed
rollback are recorded in [FIRST_PHYSICAL_BOOT.md](FIRST_PHYSICAL_BOOT.md).
