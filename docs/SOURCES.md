...
| [Qualcomm LK partition type map](https://android.googlesource.com/kernel/lk/+/9d6e28f24ba01e563d0b3801c6c6c95d756db623/platform/msm_shared/include/partition_parser.h) | Android LK commit `9d6e28f24ba01e563d0b3801c6c6c95d756db623`, checked 2026-09-14 | MBR types `0x4d` SBL1, `0x46` TZ, `0x4a` modem_st1, `0x4b` modem_st2 and `0x48` boot. | `VERIFIED_UPSTREAM` for Qualcomm type meanings; exact Hikari payload bytes were not inspected. |
| [AOSP LT26 init.lt26.rc](https://android.googlesource.com/device/sony/lt26/+/a70157d18d9ffc457de1789d29e19f3ab8a5534f/config/init.lt26.rc) | AOSP/Sony LT26 commit `a70157d18d9ffc457de1789d29e19f3ab8a5534f`, checked 2026-09-14 | p10 is mounted at `/data/idd`; `tad` receives p1. | `VERIFIED_VENDOR_SOURCE`, independently matching exact-device legacy runtime evidence. |
...
| OpenSEMC Fuji persistent console | Local historical checkout `c4784b04c08d30f799b8b14b597aeb2124d2e6e1`, rechecked 2026-09-06: `arch/arm/mach-msm/board-semc_fuji.c:3947-3965,4606-4628`; `drivers/staging/android/persistent_ram.c`; `drivers/staging/android/ram_console.c` | Legacy Fuji uses `0x7ffe0000+0x20000`, `DBGC`, an ARM32 12-byte ring header, and calls the ring with ECC enabled. Zero platform fields select defaults: data block 128, parity 16, symbol 8, polynomial `0x11d`; valid old content is exported through `/proc/last_kmsg`. | `HISTORICAL_SOURCE`, joined to physical TWRP invalid-full-ring evidence for the recovery reader format. |
| Linus Linux ramoops core and binding | Local upstream checkout `786262be6048deab760f68c8acc2c85607165894` plus project kernel `efae8bd73882339f2f6a8a394479d6c0eeeeedfa`, rechecked 2026-09-06: `fs/pstore/ram_core.c`, `fs/pstore/ram.c`, `Documentation/devicetree/bindings/reserved-memory/ramoops.yaml` | Current upstream uses the same `DBGC` header and the same Reed-Solomon geometry/defaults when `ecc-size = <16>`; a console-only zone is supported. | `VERIFIED_UPSTREAM` for the corrected ramoops binary-layout comparison. |
...
| [Debian 13 “trixie”](https://www.debian.org/releases/trixie/) | stable 13.7, checked 2026-09-14; repository suite pinned as `trixie` | Debian package licences | Current stable native userspace; official release page lists armhf as supported | Rootfs source for the Hikari Debian architecture; downloaded from a Debian mirror only after validating its signed Release metadata with Debian's archive keyring. |
...
| OpenSEMC Fuji USB BSP | `board-semc_fuji.c`, `devices-msm8x60.c`, `charger-fuji_hikari.c`, `include/linux/mfd/pm8xxx/pm8901.h` and `arch/arm/mach-msm/include/mach/board-msm8660.h` in [OpenSEMC MSM8x60](https://github.com/OpenSEMC/android_kernel_sony_msm8x60/tree/c4784b04c08d30f799b8b14b597aeb2124d2e6e1), branch `kk_chocolate_rmfx`, rechecked 2026-09-11 | HSUSB at `0x12500000`; PM8058 L6 3.05 V and L7 1.8 V. The BSP explicitly defines PMIC GPIO/MPP namespace bases as zero-based: ID index 30 maps to physical PM8058 GPIO31 and VBUS index 10 maps to physical PM8058 MPP11; ID has a 1.5 kOhm/S3 pull-up and VBUS is inverted in software. PM8901 index 0 maps to physical MPP1 external 5 V. PM8901 is the second SSBI at `0x00c00000`, IRQ TLMM91, four MPPs; OTG source order is BQ24160 OTG lock -> PM8901 MPP1 external 5 V -> NCP373 enable TLMM28, with active-low fault TLMM104. | `HISTORICAL_SOURCE`; exact board wiring, namespace translation and sequencing evidence used by the local dual-role DT and charger guard. Build 0079 adds `VERIFIED_DEVICE` target-Linux OTG power, enumeration, teardown and return-to-device evidence. |
...
| Sony/OpenSEMC MSM8x60 IOMMU description | [`devices-iommu.c`](https://github.com/OpenSEMC/android_kernel_sony_msm8x60/blob/c4784b04c08d30f799b8b14b597aeb2124d2e6e1/arch/arm/mach-msm/devices-iommu.c), revision `c4784b04c08d30f799b8b14b597aeb2124d2e6e1` | Hikari/Fuji MDP has two IOMMU providers, two context banks per provider, and non-secure MIDs 0 and 2 on both ports. | `HISTORICAL_SOURCE`; exact Sony-generation provider/context/MID evidence used by the Hikari DT. |
| Current Tenderloin MSM8x60 IOMMU port | [`msm_iommu.c`](https://github.com/shr-distribution/linux/blob/37d5887cd67dae88b0eb37564adc8e6c0c39c609/drivers/iommu/msm_iommu.c), branch `6.18`, head `37d5887cd67dae88b0eb37564adc8e6c0c39c609`, checked 2026-09-06 | Uses provider-local client-master lookup for devices spanning multiple MSM8x60 IOMMUs instead of one per-device private pointer. | `HISTORICAL_SOURCE`; current physically used MSM8x60 port architecture reference for the deferred-probe fix, not Hikari wiring evidence. |
...
| OpenSEMC Fuji BCM4330 WLAN BSP | Local `opensemc-msm8x60` checkout `c4784b04c08d30f799b8b14b597aeb2124d2e6e1`, rechecked 2026-09-16: `arch/arm/mach-msm/board-semc_fuji.c:3880-3936,6100-6121,6810-6822,6913-6923`; `arch/arm/mach-msm/gpiomux-fuji_hikari.c:946-966`; `board-semc_fuji.c:4785-4797` | BCM4330 on non-removable four-bit SDCC4 at 400 kHz--48 MHz; TLMM128 HOST_WAKE, TLMM130 WL_RST_N; physical PM8058 GPIO38 FUNC2 is the low-drive, no-pull 32.768 kHz sleep clock using the S3 input domain; physical SDCC4 supply is PM8058 S3 at 1.8 V although the host advertises 2.7--2.9 V OCR. | `HISTORICAL_SOURCE`; exact board wiring. Joined with `VERIFIED_DEVICE` SDIO enumeration and scan evidence on 2026-09-16. |
| Hikari Android device firmware manifest | Local historical checkout `android_device_sony_hikari`: `hikari.mk:69-71`, `proprietary-files.txt:30,69`, rechecked 2026-09-16 | Identifies the BCM4330 B2 board variant and stock Bluetooth/calibration paths. The owner's installed stock system additionally contains the matching B2 WLAN firmware. | `HISTORICAL_SOURCE`; filenames/provenance only. Firmware and calibration are private inputs and are not committed or redistributed. |
