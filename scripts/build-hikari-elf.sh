...
# package both display and USB-safe Sony ELF32 artifacts from a caller-supplied
...
[[ -f $rpm ]] || { echo 'RPM_PAYLOAD must name the private/appropriately licensed LT26 RPM firmware ELF' >&2; exit 1; }
...
for profile in display safe; do
  case "$profile" in
    display)
...
(
  cd "$artifact_dir"
  find common display safe -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS
)
printf 'HIKARI_FASTBOOT_ARTIFACTS=PASS\ndisplay=%s\nsafe=%s\n' "$display_output" "$safe_output"
