...
for include_name in "$hardware_name" "$wireless_name"; do
	if ! rg -q "^#include \"${include_name//./\\.}\"$" "$target_dir/qcom-msm8260-sony-hikari.dts"; then
		printf '\n#include "%s"\n' "$include_name" >> "$target_dir/qcom-msm8260-sony-hikari.dts"
	fi
done
...
echo "prepared $kernel_src with qcom-msm8260-sony-hikari.dtb, $hardware_name and $wireless_name"
