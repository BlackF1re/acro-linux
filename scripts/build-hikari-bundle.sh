...
# Build the reproducible, redistributable Hikari development bundle.  No RPM
# firmware is embedded here and no device transport is invoked.
set -euo pipefail
...
# Prime usr/gen_init_cpio, then build the canonical static BusyBox initramfs,
# then rebuild zImage with that exact archive embedded.
...
HIKARI_BUILD_ROOT="$hikari_build_root" KERNEL_SRC="$kernel_src" \
  ...
  TARGETS='zImage qcom/qcom-msm8260-sony-hikari.dtb qcom/qcom-msm8260-sony-hikari-safe.dtb' \
  REQUIRE_USB_DEBUG="$require_usb_debug" \
  REQUIRE_DISPLAY_BRINGUP="$require_display_bringup" \
  REQUIRE_CHARGING="$require_charging" \
  "$repo_root/scripts/build-hikari-kernel.sh"
...
for input in "$zimage" "$dtb_display" "$dtb_safe" "$initramfs" "$kernel_build/.config"; do
...
mkdir -p "$artifact_dir/display" "$artifact_dir/safe" "$artifact_dir/common"
...
for path in (root/'display/hikari-zImage-appended-dtb', root/'safe/hikari-zImage-appended-dtb'):
...
profile_display=full Hikari hardware and experimental display bring-up
profile_safe=USB/ramoops diagnostic fallback with multimedia island disabled
fastboot_ready=no (RPM firmware intentionally not embedded in this public bundle)
EOF
(
  cd "$artifact_dir"
  find common display safe -type f -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS
)
